    ...
    if soup.body:
        for i in soup.body.find_all('div', attrs={'class': "row clear"}):
            ...
            if name in ('Filename', 'MD5 Hash'):
                result[name] = value
    try:
        download_elem = soup.find(id='downloadmirrorstoggle')
        if download_elem is not None and hasattr(download_elem, 'attrs'):
            href = download_elem.attrs.get('href')  # type: ignore[union-attr]
            if isinstance(href, str):
                result['Download'] = href.strip()
...
class ModDBDownloader(DefaultDownloader):
    ...
    def check(self, dl_dir: Path, update_cache: bool = False) -> bool:  # noqa: C901
        if not self._iurl:
            raise HashError(f'No info_url related to mod')
        ...
        super().download(dl_dir)
        super().download(dl_dir, use_cached=True, hash=hash or "")  # to check hash
        return True
    ...
    def download(self, to: Path, use_cached: bool = False, *args, **kwargs) -> Path:
        ...
        hash: Optional[str] = metadata.get('MD5 Hash', None) if metadata.get('Download', '') in self._url else None

        self._url = self._get_download_url(self._url)
        self._archive = to / os.path.basename(urlparse(self._url).path)
        return super().download(to, use_cached, hash=hash or "")
