...
class Usvfs:
    ...
    arguments: dict = {
        ...
        '--final': {'help': 'Path to final install directory', 'required': True, 'type': str},
    }
    ...
    def _read_modlist(modlist: Path) -> Iterator[str]:
        mods = [i.lstrip('+') for i in filter(lambda x: x.startswith('+'), modlist.read_text().split('\n'))]
    ...
    def run(self, args) -> None:
        ...
        for mod in self._read_modlist(gamma_dir / 'profiles' / 'G.A.M.M.A' / 'modlist.txt'):
            ...
            try:
                copy_tree(str(gamma_dir / 'mods' / mod), str(install_dir))
            except DistutilsFileError as err:
                print(f'    --> Failed: {err}')

        print('Reapplying Anomaly binary dir to install directory')
        copy_tree(str(anomaly_dir / 'bin'), str(install_dir / 'bin'))
